<?php
	...
	class Nepali_Calendar
	{
		...
		private function _is_in_range_eng($yy, $mm, $dd)
		{
			if ($yy < 1944 || $yy > 2033)
			{
				return 'Supported only between 1944 AD - 2033 AD';
			}
		...
		private function _is_in_range_nep($yy, $mm, $dd)
		{
			if ($yy < 2000 || $yy > 2089)
			{
				return 'Supported only between 2000 BS - 2089 BS';
			}
		...
		public function get_data($yy)
		{
			for ($i = 0 ; $i < count ($this->_bs); $i++) 
			{
				if (in_array($yy, $this->_bs[$i])) {return $this->_bs[$i];}				
			}
		...
		{
			return $this->_get_nepali_month($m);
		}
